...
async function locateScalpel(nest: any) {
  let currentNest: string = nest.name;
  let nextNest: string;
  try {
    while (true) {
      nextNest = (await anyStorage(nest, currentNest, "scalpel")) as string;
...
function locateScalpel2(nest: any) {
  let currentNest: string = nest.name;
  function trackScalpel(currentNest: string): any {
    try {
      return anyStorage(nest, currentNest, "scalpel").then(
        (nextNest: string) => {
          if (nextNest === currentNest) {
            return currentNest;
          } else {
            return trackScalpel(nextNest);
          }
        }
      );
    } catch (e) {
